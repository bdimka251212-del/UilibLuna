...
Library.Round = function(self, Number, Float)
    ...
    local f = type(Float) == "number" and Float or 1
    if f <= 0 or f ~= f or f == math.huge or f == -math.huge then
        f = 1
    end
...
do 
    ...
    Library.Watermark = function(self, Data)
        ...
        local Config = {
            Title = Data.Title or "saladware",
            UID = Data.UID or "1",
            ShowFPS = Data.ShowFPS ~= false,
            ShowPing = Data.ShowPing ~= false,
            UpdateRate = Data.UpdateRate or 0.5
        }
        
        function Watermark:SetUID(UID)
            Config.UID = tostring(UID)
        end
        ...
        function Watermark:SetStats(UID, FPS, Ping)
            local TextParts = {Config.Title}
            if UID then
                table.insert(TextParts, "| UID: " .. tostring(UID))
            end
            if FPS and Config.ShowFPS then
                table.insert(TextParts, "| FPS: " .. tostring(FPS))
            end
            if Ping and Config.ShowPing then
                table.insert(TextParts, "| Ping: " .. tostring(Ping) .. "ms")
            end
            Watermark:SetText(table.concat(TextParts, " "))
        end
        ...
        if Config.ShowFPS or Config.ShowPing then
            ...
            Library:Connect(RunService.RenderStepped, function()
                ...
                if CurrentTime - LastUpdate >= Config.UpdateRate then
                    ...
                    Watermark:SetStats(Config.UID, Config.ShowFPS and FPS or nil, Config.ShowPing and Ping or nil)
                end
            end)
        else
            Watermark:SetStats(Config.UID)
        end
        ...
        do
            ...
            frame.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    local parentAbs = parent and parent.AbsolutePosition or Vector2New(0, 0)
                    local abs = frame.AbsolutePosition
                    frame.AnchorPoint = Vector2New(0, 0)
                    startOffset = Vector2New(abs.X - parentAbs.X, abs.Y - parentAbs.Y)
                    frame.Position = UDim2New(0, startOffset.X, 0, startOffset.Y)
            ...
            UserInputService.InputChanged:Connect(function(input)
                if dragging and input.UserInputType == Enum.UserInputType.MouseMovement and startOffset then
                    local delta = input.Position - dragStart
                    local parent = frame.Parent
                    local parentSize = parent and parent.AbsoluteSize or Camera.ViewportSize
                    local frameSize = frame.AbsoluteSize
                    local x = MathClamp(startOffset.X + delta.X, 0, math.max(0, parentSize.X - frameSize.X))
                    local y = MathClamp(startOffset.Y + delta.Y, 0, math.max(0, parentSize.Y - frameSize.Y))
                    frame.Position = UDim2New(0, x, 0, y)
                end
            end)
        end

        Watermark:SetText(Config.Title)
    ...
    Library.Sections.Slider = function(self, Data)
        ...
        local Slider = {
            ...
            Decimals = (Data.Decimals ~= nil and Data.Decimals)
                or (Data.decimals ~= nil and Data.decimals)
                or 1,
        ...
        function Slider:Set(Value)
            ...
            if type(v) ~= "number" or v ~= v or v == math.huge or v == -math.huge then
                v = (type(Slider.Default) == "number" and Slider.Default) or Slider.Min
            end
            local range = (Slider.Max - Slider.Min)
            if range == 0 or range ~= range then
                v = (type(Slider.Default) == "number" and Slider.Default) or Slider.Min
            end
            local rounded = Library:Round(v, Slider.Decimals)
            v = MathClamp(rounded, Slider.Min, Slider.Max)
            ...
            local fill = (Slider.Value - Slider.Min) / (Slider.Max - Slider.Min)
            if fill ~= fill or fill == math.huge or fill == -math.huge then fill = 0 end
            if Slider.Max > Slider.Min then
                fill = MathClamp(fill, 0, 1)
            else
                fill = 0
            end
            ...
            if Slider.Max > Slider.Min then
                local visFill = fill
                if visFill <= 0 and Slider.Value <= Slider.Min then
                    visFill = 0.06
                elseif visFill > 0 and visFill < 0.03 then
                    visFill = 0.03
                end
                accent.Visible = true
                accent.Size = UDim2New(MathClamp(visFill, 0, 1), 0, 1, -2)
            else
        ...
        if type(Slider.Default) == "number" then
